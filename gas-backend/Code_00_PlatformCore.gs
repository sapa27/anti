...
var __APP_GLOBAL__ = (function () {
    ...
    stamp: "commission-v1.2-gas-backend-single-source-2026-08-09-r253",
    channel: "dual-host",
    buildName: "V1.2 GAS Backend Single Source r253",
    releaseDate: "2026-08-09",
    assetStamp: "asset-manifest-r253-gas-backend-single-source",
    sourceFingerprint: "gas-backend-single-source-r253",
    contractStamp: "current-quality-gate-r253",
...
var APP_COMPATIBILITY_CONTRACT_CURRENT = Object.freeze({
    stamp: "compatibility-contract-current-r253",
...
var APP_PERFORMANCE_GATE_CURRENT = Object.freeze({
    stamp: "performance-gate-current-r253", owner: "Code_00_PlatformCore.AppPerformanceGateCurrent", targets: Object.freeze({
...
var APP_RELEASE_POLICY_CURRENT = Object.freeze({
    stamp: "release-policy-current-r253", owner: "Code_00_PlatformCore.AppReleasePolicyCurrent", policy: "release-manifest + current-quality-gate + correlated performance/resource/UX evidence + exactly-once refresh + concurrent-write operator gate", requiredRoutes: Object.freeze([
...
var APP_CONTRACT_FREEZE_CURRENT = Object.freeze({
    stamp: "contract-freeze-production-lock-current-r253", owner: "Code_00_PlatformCore.PhaseAContractFreeze", contracts: [
...
function _normalizeGitHubPagesOrigin_(value) {
    value = String(value || "").trim();
    var match = value.match(/^(https?:\/\/[^\/?#\s]+)/i);
    if (!match)
        return "";
    var origin = String(match[1] || "").replace(/\/$/, "");
    if (/^https:\/\/[a-z0-9.-]+(?::\d+)?$/i.test(origin) || /^http:\/\/localhost(?::\d+)?$/i.test(origin))
        return origin.toLowerCase();
    return "";
}
var APP_GITHUB_PAGES_CANONICAL_ORIGIN = "https://sapa27.github.io";
function _githubPagesAllowedOrigins_() {
    var raw = "";
    try {
        raw = String(_scriptProp_("GITHUB_PAGES_ORIGINS", "") || _scriptProp_("GITHUB_PAGES_ORIGIN", "") || "").trim();
    }
    catch (_e) {
        raw = "";
    }
    var out = {};
    var canonicalOrigin = _normalizeGitHubPagesOrigin_(APP_GITHUB_PAGES_CANONICAL_ORIGIN);
    if (canonicalOrigin)
        out[canonicalOrigin] = !0;
    raw.split(/[\s,;]+/).forEach(function (value) {
        var origin = _normalizeGitHubPagesOrigin_(value);
        if (origin)
            out[origin] = !0;
    });
    return out;
}
function _githubPostMessageRelaySource_() {
    return 'function send(w){if(!w||typeof w.postMessage!=="function")return false;try{w.postMessage(m,target);return true}catch(_e){return false}}';
}
function _githubPagesBridgeNonce_(value) {
    value = String(value || "").trim();
    return /^[A-Za-z0-9_-]{16,128}$/.test(value) ? value : "";
}
function _renderGitHubBridgeError_(parentOrigin, nonce, code, configuredOrigins) {
    parentOrigin = _normalizeGitHubPagesOrigin_(parentOrigin) || String(parentOrigin || "").trim().replace(/\/$/, "");
    nonce = String(nonce || "").trim();
    code = String(code || "GAS_BRIDGE_DENIED").replace(/[^A-Z0-9_-]/gi, "");
    configuredOrigins = Array.isArray(configuredOrigins) ? configuredOrigins : [];
    var safeTarget = _normalizeGitHubPagesOrigin_(parentOrigin) || "*";
    var diagnostic = {
        type: "GAS_BRIDGE_ERROR", ok: !1, code: code, nonce: nonce,
        bridgeVersion: "github-pages-bridge-v1",
        requestedOrigin: parentOrigin, configuredOrigins: configuredOrigins
    };
    var html = '<!doctype html><html><head><meta charset="utf-8"><title>Bridge denied</title></head>' +
        '<body><pre id="bridge-error" style="white-space:pre-wrap;font:12px system-ui"></pre>' +
        '<script>(function(){var m=' + JSON.stringify(diagnostic) + ';var target=' + JSON.stringify(safeTarget) + ';' +
        _githubPostMessageRelaySource_() +
        'try{send(window.top)}catch(e){}try{send(window.parent)}catch(e){}' +
        'var el=document.getElementById("bridge-error");if(el)el.textContent=m.code+"\nrequested="+(m.requestedOrigin||"")+"\nconfigured="+(m.configuredOrigins||[]).join(",");' +
        '})();<\/script></body></html>';
    return HtmlService.createHtmlOutput(html).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
function _renderGitHubPagesBridge_(e) {
    var params = e && e.parameter || {}, parentOrigin = _normalizeGitHubPagesOrigin_(params.parentOrigin), rawNonce = String(params.nonce || "").trim(), nonce = _githubPagesBridgeNonce_(rawNonce), allowed = _githubPagesAllowedOrigins_(), configuredOrigins = Object.keys(allowed);
    if (!parentOrigin || !allowed[String(parentOrigin).toLowerCase()])
        return _renderGitHubBridgeError_(parentOrigin || params.parentOrigin, rawNonce, "GITHUB_PAGES_ORIGIN_NOT_ALLOWED", configuredOrigins);
    if (!nonce)
        return _renderGitHubBridgeError_(parentOrigin, rawNonce, "GITHUB_PAGES_BRIDGE_NONCE_INVALID", configuredOrigins);
    var template = HtmlService.createTemplateFromFile("GitHub_Bridge");
    template.parentOriginJson = JSON.stringify(parentOrigin);
    template.bridgeNonceJson = JSON.stringify(nonce);
    return template.evaluate()
        .setTitle("GAS GitHub Bridge")
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
function _githubApiPostRequestId_(value) {
    value = String(value || "").trim();
    return /^[A-Za-z0-9_.:-]{1,160}$/.test(value) ? value : "";
}
function _githubApiPostSafeError_(error) {
    var message = "GAS API POST request failed", code = "GAS_POST_REQUEST_FAILED";
    try {
...
function _renderGitHubApiPostResult_(parentOrigin, nonce, requestId, ok, result, error, meta) {
    parentOrigin = _normalizeGitHubPagesOrigin_(parentOrigin) || String(parentOrigin || "").trim().replace(/\/$/, "");
    var targetOrigin = _normalizeGitHubPagesOrigin_(parentOrigin) || "*";
    var envelope = {
        type: "GAS_POST_RESULT",
        transportVersion: "github-postmessage-v1",
        nonce: String(nonce || ""),
        id: String(requestId || ""),
        ok: !!ok,
        result: ok ? result : null,
        error: ok ? null : (error || { message: "GAS API POST request failed", code: "GAS_POST_REQUEST_FAILED" }),
        meta: meta || null
    };
    var payloadJson = "{}";
    try {
        ...
        payloadJson = JSON.stringify({
            type: "GAS_POST_RESULT", transportVersion: "github-postmessage-v1",
            nonce: String(nonce || ""), id: String(requestId || ""), ok: !1, result: null,
            error: { message: "ไม่สามารถแปลงผลลัพธ์ GAS เป็น JSON ได้", code: "GAS_POST_RESULT_SERIALIZE_FAILED" }
        });
    }
    var html = '<!doctype html><html><head><meta charset="utf-8"><title>GAS API Result</title></head><body>' +
        '<script>(function(){var m=' + payloadJson + ';var target=' + JSON.stringify(targetOrigin) + ';' +
        _githubPostMessageRelaySource_() +
        'var w=window;for(var i=0;i<6;i++){try{var p=w.parent;if(!p||p===w)break;send(p);w=p}catch(e){break}}' +
        'try{send(window.top)}catch(e){}try{send(window.opener)}catch(e){}' +
        '})();<\/script></body></html>';
    return HtmlService.createHtmlOutput(html).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
function _invokeGitHubApiPost_(fn, args) {
    fn = String(fn || "").trim();
    args = args == null ? {} : args;
    ...
    throw Object.assign(new Error("POST transport function not allowed: " + fn), { code: "GAS_POST_FN_DENIED" });
}
function _handleGitHubApiPost_(e) {
    var params = e && e.parameter || {};
    var parentOrigin = _normalizeGitHubPagesOrigin_(params.parentOrigin);
    var rawOrigin = String(params.parentOrigin || "").trim();
    var nonce = _githubPagesBridgeNonce_(params.nonce);
    var requestId = _githubApiPostRequestId_(params.id);
    var fn = String(params.fn || "").trim();
    var allowedOrigins = _githubPagesAllowedOrigins_();
    var configuredOrigins = Object.keys(allowedOrigins);
    if (!parentOrigin || !allowedOrigins[String(parentOrigin).toLowerCase()]) {
        return _renderGitHubApiPostResult_(parentOrigin || rawOrigin, nonce, requestId, !1, null, {
            message: "GitHub Pages origin ไม่ได้รับอนุญาต",
            code: "GITHUB_PAGES_ORIGIN_NOT_ALLOWED"
        }, { requestedOrigin: rawOrigin, configuredOrigins: configuredOrigins });
    }
    if (!nonce) {
        return _renderGitHubApiPostResult_(parentOrigin, "", requestId, !1, null, {
            message: "GitHub API POST nonce ไม่ถูกต้อง", code: "GITHUB_POST_NONCE_INVALID"
        }, { requestedOrigin: parentOrigin });
    }
    if (!requestId) {
        return _renderGitHubApiPostResult_(parentOrigin, nonce, "", !1, null, {
            message: "GitHub API POST request id ไม่ถูกต้อง", code: "GITHUB_POST_REQUEST_ID_INVALID"
        }, null);
    }
    var args = {};
    try {
        ...
        return _renderGitHubApiPostResult_(parentOrigin, nonce, requestId, !1, null, {
            message: "GitHub API POST payload ไม่ใช่ JSON ที่ถูกต้อง", code: "GITHUB_POST_PAYLOAD_INVALID"
        }, null);
    }
    try {
        var result = _invokeGitHubApiPost_(fn, args);
        return _renderGitHubApiPostResult_(parentOrigin, nonce, requestId, !0, result, null, {
            fn: fn, transport: "github-postmessage", release: APP_DEPLOY_RELEASE.stamp
        });
    }
    catch (error) {
        return _renderGitHubApiPostResult_(parentOrigin, nonce, requestId, !1, null, _githubApiPostSafeError_(error), {
            fn: fn, transport: "github-postmessage", release: APP_DEPLOY_RELEASE.stamp
        });
    }
}
function doPost(e) {
    ...
    if (mode === "github-api-post") return _handleGitHubApiPost_(e);
    return HtmlService.createHtmlOutput("Unsupported POST mode").setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
function doGet(e) {
    var mode = String(e && e.parameter && e.parameter.mode || "").trim().toLowerCase();
    return mode === "github-bridge" ? _renderGitHubPagesBridge_(e) : renderVue3App_(e);
}
...
var ROUTER_PLATFORM_FACADE_STAMP_CURRENT = "router-platform-observability-owner-current-r253";
