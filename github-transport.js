(function (root, doc) {
  ...
  var bridgeLastError = "";
  var EXPECTED_BRIDGE_VERSION = text(config.BRIDGE_VERSION || "github-pages-bridge-v2");
  var FRONTEND_TRANSPORT_VERSION = "github-bridge-first-v2-nav-lifecycle";
  ...
  var transportMetrics = {
    ...
    apiBridgeFirstCalls: 0,
    apiBridgeReadyFailures: 0,
    apiPostFallbackCalls: 0
  };
    ...
    readyPromise = new Promise(function (resolve, reject) {
      var timeout = root.setTimeout(function () {
        ...
        var hint = state === "loaded-no-ready"
          ? "Bridge iframe โหลดแล้วแต่ช่องสื่อสารจาก GAS sandbox ไม่เชื่อมต่อ: ตรวจว่า GAS deploy จาก canonical r257 และ GITHUB_PAGES_ORIGIN ตรงกับ location.origin"
      ...
      function onReady(event) {
        ...
        bridgeChannel = text(data.channel || "window-postmessage");
  ...
  function tryLocalDeferred(fn, args) {
    ...
    return localDeferredResponse(req.name).then(function (response) {
      ...
      if (root.console && console.warn) console.warn("[r257] local asset fallback to GAS", req.name, error && error.message || error);
      return null;
    });
  }
  ...
  function bridgeFirstRun(invocation, options, authRequest) {
    if (authRequest) transportMetrics.authBridgeFirstCalls += 1;
    else transportMetrics.apiBridgeFirstCalls += 1;
    return bridgeRun(invocation.fn, invocation.args, options).catch(function (bridgeError) {
      /* Fallback is allowed only when the persistent bridge was unavailable BEFORE
         a GAS API call could be dispatched. Never retry GAS_BRIDGE_REQUEST_TIMEOUT
         or server/API errors through POST, because writes must remain exactly-once. */
      ...
      if (authRequest) {
        ...
        transportMetrics.apiBridgeReadyFailures += 1;
        transportMetrics.apiPostFallbackCalls += 1;
      }
      if (root.console && console.warn) console.warn("[r257] bridge unavailable before dispatch; fallback to POST", bridgeError && bridgeError.code || "", bridgeError && bridgeError.message || bridgeError);
      var fallbackTimeout = Number(authRequest ? (config.AUTH_POST_FALLBACK_TIMEOUT_MS || 15000) : (config.API_POST_FALLBACK_TIMEOUT_MS || 20000));
      return postRun(invocation.fn, invocation.args, optionsWithTimeout(options, fallbackTimeout)).catch(function (postError) {
        var prefix = authRequest ? "ไม่สามารถเชื่อมต่อ GAS สำหรับเข้าสู่ระบบได้" : "ไม่สามารถเชื่อมต่อ GAS สำหรับโหลด/บันทึกข้อมูลได้";
        var message = prefix +
          " — Bridge: " + text(bridgeError && bridgeError.message || bridgeError) +
          " — POST: " + text(postError && postError.message || postError) +
          " — ตรวจ GAS deployment, GitHub origin และ Web App permission";
        var combined = createError({ message: message, code: authRequest ? "GAS_AUTH_TRANSPORT_UNAVAILABLE" : "GAS_API_TRANSPORT_UNAVAILABLE" });
  ...
  function canonicalRemoteRun(fn, args, options) {
    var invocation = normalizeBridgeInvocation(fn, args);
    var authRequest = !!AUTH_BRIDGE_FUNCTIONS[invocation.fn];
    var authMode = text(config.AUTH_TRANSPORT_MODE || "bridge-first").toLowerCase();
    var apiMode = text(config.API_TRANSPORT_MODE || "bridge-first").toLowerCase();
    var mode = authRequest ? authMode : apiMode;
    if (mode === "bridge-first") return bridgeFirstRun(invocation, options, authRequest);
    if (mode === "bridge") {
  ...
  function run(fn, args, options) {
    ...
    if (localAttempt) {
      return Promise.resolve(localAttempt).then(function (localResponse) {
        ...
        return canonicalRemoteRun(fn, args, options);
      });
    }
    return canonicalRemoteRun(fn, args, options);
  }
  ...
  root.AppTransport.mode = "github-pages-gas-router-bridge-first";
  ...
  root.setTimeout(function () {
    var authBridgeFirst = text(config.AUTH_TRANSPORT_MODE || "bridge-first").toLowerCase() === "bridge-first";
    var apiBridgeFirst = text(config.API_TRANSPORT_MODE || "bridge-first").toLowerCase() === "bridge-first";
    if (authBridgeFirst || apiBridgeFirst) root.AppTransport.warmAuthBridge();
  }, 0);
  root.AppTransport.status = function () {
    return {
      ...
      apiTransportMode: text(config.API_TRANSPORT_MODE || "bridge-first"),
